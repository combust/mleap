...
object MleapBundle {
  def readTransformerGraph(path: File)
                          (implicit context: MleapContext = MleapContext()): (Bundle, Pipeline) = {
  ...
  def readTransformer(path: File)
                     (implicit context: MleapContext = MleapContext()): (Bundle, Transformer) = {
    val bundle = BundleSerializer(context, path).read()
  ...
  def writeTransformerGraph(graph: Pipeline,
                           ...
                           (implicit context: MleapContext = MleapContext()): Unit = {
