...
INPUT_SCHEMA = StructType([StructField("f1", FloatType()),])
...
class MathUnaryTest(unittest.TestCase):
    ...
    def setUp(self):
        self.input = self.spark.createDataFrame(
            [(float(i),) for i in range(1, 10)], INPUT_SCHEMA
        )

        self.expected_sin = pd.DataFrame(
            [(math.sin(i),) for i in range(1, 10)], columns=["sin(f1)"],
        )
    ...
    def test_sin_math_unary(self):
        sin_transformer = MathUnary(
            operation=UnaryOperation.Sin, inputCol="f1", outputCol="sin(f1)",
        )
    ...
    def test_math_unary_pipeline(self):
        ...
        exp_transformer = MathUnary(
            operation=UnaryOperation.Exp, inputCol="sin(f1)", outputCol="exp(sin(f1))",
        )

        expected = pd.DataFrame(
            [(math.exp(math.sin(i)),) for i in range(1, 10)], columns=["exp(sin(f1))"],
        )

        pipeline = Pipeline(stages=[sin_transformer, exp_transformer])
    ...
    def test_can_instantiate_all_math_unary(self):
        for unary_operation in UnaryOperation:
            transformer = MathUnary(
                operation=unary_operation, inputCol="f1", outputCol="operation",
            )
    ...
    def test_serialize_deserialize_math_unary(self):
        ...
        file_path = "{}{}".format(
            "jar:file:", os.path.join(self.tmp_dir, "math_unary.zip")
        )

        sin_transformer.serializeToBundle(file_path, self.input)
        deserialized_math_unary = SimpleSparkSerializer().deserializeFromBundle(
            file_path
        )
    ...
    def test_serialize_deserialize_pipeline(self):
        ...
        file_path = "{}{}".format(
            "jar:file:", os.path.join(self.tmp_dir, "math_unary_pipeline.zip")
        )
