...
class StringMapTest(unittest.TestCase):
    ...
    def test_map(self):
        result = StringMap(
            labels={"a": 1.0}, inputCol="key_col", outputCol="value_col",
    ...
    def test_map_custom_default_value(self):
        ...
        expected = StringMapTest.spark.createDataFrame(
            [["a", "b", -1.0]], OUTPUT_SCHEMA
        )
        assert_df(expected, result)
    ...
    def test_map_missing_value_error(self):
        with self.assertRaises(Py4JJavaError) as error:
            StringMap(
                labels={"z": 1.0}, inputCol="key_col", outputCol="value_col"
            ).transform(self.input).collect()
        self.assertIn(
            "java.util.NoSuchElementException: Missing label: a", str(error.exception)
        )

    def test_map_from_dataframe(self):
        labels_df = StringMapTest.spark.createDataFrame(
            [["a", 1.0]], "key_col: string, value_col: double"
        )
        result = StringMap.from_dataframe(
            labels_df=labels_df, inputCol="key_col", outputCol="value_col"
        ).transform(self.input)
        expected = StringMapTest.spark.createDataFrame([["a", "b", 1.0]], OUTPUT_SCHEMA)
        assert_df(expected, result)
    ...
    def test_validate_handleInvalid_bad(self):
        with self.assertRaises(AssertionError):
            StringMap(
                labels=None, inputCol=dict(), outputCol=None, handleInvalid="invalid"
            )
    ...
    def test_validate_labels_value_fails(self):
        with self.assertRaises(AssertionError):
            StringMap(
                labels=None,
                inputCol={"valid_key_type": "invalid_value_type"},
                outputCol=None,
            )
