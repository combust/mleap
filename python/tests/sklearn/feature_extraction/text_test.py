...
class TransformerTests(unittest.TestCase):
    ...
    def test_countvectorizer_serializer(self):
        ...
        with open(
            "{}/{}.node/model.json".format(self.tmp_dir, count.name)
        ) as model_json:
    ...
    def test_tfidf_vectorizer_serializer_tf_part(self):
        ...
        with open(
            "{}/{}/root/{}.node/model.json".format(
                self.tmp_dir, self.tfidf.name, tf_node_name
            )
        ) as tf_model_json:
    ...
    def test_tfidf_vectorizer_serializer_idf_part(self):
        ...
        with open(
            "{}/{}/root/{}.node/node.json".format(
                self.tmp_dir, self.tfidf.name, idf_node_name
            )
        ) as idf_node_json:
    ...
    def load_pipeline_model(self):
        with open(
            "{}/{}/root/model.json".format(self.tmp_dir, self.tfidf.name)
        ) as pipeline_model_json:
